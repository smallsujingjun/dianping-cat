...
<p class="text-error">由于计算95line需要数据较多，为了减少内存开销，系统做了一些优化，在单个小时内某一台机器，95Line误差是1ms，但是合并成1一天，1周，1个月误差较大（数据仅供参考）</p>
...
<img  class="img-polaroid"  src="${model.webapp}/images/transaction03.jpg"/>
<br/>
<h4 class="text-success">Transaction历史统计报表</h4>
<p class="text-error">a）Transaction\Event月报表支持每天的趋势图，以天为单位,如下图</p>
<img  class="img-polaroid"  src="${model.webapp}/images/transaction06.png"/>
<p class="text-error">b）Transaction\Event报表日报表、周报表支持同比、环比对比,时间精度为5分钟</p>
<img  class="img-polaroid"  src="${model.webapp}/images/transaction05.png"/>